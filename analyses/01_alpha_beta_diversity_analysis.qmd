...
apc_summary <- apc_effect_data %>%
  select(infection_status, R2, p_value, p_adj, sig, variance_ok) %>%
  mutate(
...
caga_summary <- caga_effect_data %>%
  select(genetic_background, R2, p_value, p_adj, sig, variance_ok) %>%
  mutate(
    R2_percent = paste0(round(R2 * 100, 1), "%"),
    interpretation = case_when(
      p_adj < 0.05 ~ "CagA效应显著",
...
infection_apcmut <- part5_results %>%
  filter(category %in% c("感染效应@ApcMUT", "CagA效应") &
         grepl("ApcMUT", group1) & grepl("ApcMUT", group2))

infection_apcwt <- part5_results %>%
  filter(category %in% c("感染效应@ApcWT", "CagA效应") &
...
# 1. Apc效应分析
apc_ctrl <- part5_results %>% filter(question == "Apc基线效应 (Ctrl组)")
apc_hpko <- part5_results %>% filter(question == "Apc效应 (HpKO感染后)")
apc_hpwt <- part5_results %>% filter(question == "Apc效应 (HpWT感染后)")
...
# 2. CagA效应分析
caga_mut <- part5_results %>% filter(question == "CagA效应 (ApcMUT背景)")
caga_wt <- part5_results %>% filter(question == "CagA效应 (ApcWT背景)")
...
significant_comparisons <- part5_results %>% filter(p_adj < 0.05)
...
virus_alpha_data <- as.data.frame(colData(tse_virus_full)) %>%
  select(treatment_group, genotype_factor, infection_factor, shannon_index, observed_richness)
