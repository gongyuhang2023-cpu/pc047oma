...
# 2. 添加因子变量：genotype 和 infection
# 根据sample_name判断genotype（数字开头=ApcMUT，字母开头=ApcWT）
...
# 根据treatment_group提取infection状态
colData(tse_full)$infection <- factor(
  dplyr::case_when(
    grepl("Ctrl", colData(tse_full)$treatment_group) ~ "Ctrl",
    grepl("HpKO", colData(tse_full)$treatment_group) ~ "HpKO",
    grepl("HpWT", colData(tse_full)$treatment_group) ~ "HpWT"
  ),
...
# 3. 验证因子编码
cat("=== Part 4: 样本分组统计 ===\n")
print(table(colData(tse_full)$genotype, colData(tse_full)$infection))
cat("\n总样本数:", ncol(tse_full), "\n")
...
disp_genotype <- vegan::betadisper(dist_bc_full, colData(tse_full)$genotype)
cat("\n=== betadisper: Genotype ===\n")
disp_test_genotype <- permutest(disp_genotype, permutations = 999)
print(disp_test_genotype)

# 4. 按infection进行betadisper检验
disp_infection <- vegan::betadisper(dist_bc_full, colData(tse_full)$infection)
...
alpha_data_full <- as.data.frame(colData(tse_full)) %>%
  dplyr::select(sample_name, treatment_group, genotype, infection,
...
cat("公式: distance ~ genotype * infection\n\n")

permanova_full <- vegan::adonis2(
  dist_bc_full ~ genotype * infection,
  data = metadata_full,
  permutations = 999,
  by = "margin"
)
...
# 1. 筛选感染组样本
tse_infected <- tse_full[, colData(tse_full)$infection != "Ctrl"]
cat("=== 2×2子集分析（排除Ctrl组）===\n")
cat("样本数:", ncol(tse_infected), "\n")
print(table(colData(tse_infected)$genotype, colData(tse_infected)$infection))
