...
subtitle: "PC047 vCagAepitope - Bracken物种组成与群落结构分析"
author:
  - name: Gong Yuhang
date: today
toc: true
...
> **核心问题**：CagA蛋白是否显著改变了小鼠肠道微生物组的群落结构？
>
...
本文档是PC047项目的首个核心分析模块。目标是评估CagA依赖性感染是否重塑了小鼠盲肠微生物组的群落复杂度。

**核心比较**：
- **ApcMUT_HpWT** (n=5): Apc^Min/+突变小鼠 + H. pylori野生型感染 (CagA+)
- **ApcMUT_HpKO** (n=4): Apc^Min/+突变小鼠 + H. pylori CagA敲除感染 (CagA-)

## 预期发现

基于Functional Footprint假说，我们预期：

1. **Alpha多样性**：可能无显著差异（群落复杂度保持稳定）
2. **Beta多样性**：预期有显著差异（群落组成发生改变）
3. **物种水平 vs 病毒水平**：Standard细菌和Virus可能呈现不同的响应模式

## 方法概述

| 分析内容 | 方法 | 选择理由 |
|---------|------|----------|
| 物种组成 | Bracken (Kraken2) | 高精度物种水平分类 |
| Alpha多样性 | Shannon, Observed | 群落复杂度评估 |
| Beta多样性 | Bray-Curtis, Aitchison | 组成型数据距离度量 |
| 统计检验 | Wilcoxon, PERMANOVA | 非参数检验，适合小样本 |
| 差异丰度 | ANCOM-BC, ALDEx2 | 组成型数据DAA |
...
# 加载公共函数
source(here::here("analyses", "R", "functions.R"))
...
# Part 1: Standard细菌多样性分析 {#sec-standard}
## 1.导入数据并构建 TreeSE
```{r}
# Step 1：定义导入文件函数
...
if (nrow(daa_aldex2_data) > 0) {
  ...
  cat("✅ ALDEx2 效应量图已保存: 22_daa_aldex2_effect_size.png\n")
} else {
  cat("⚠️ 无 ALDEx2 显著差异物种，跳过可视化\n")
}
```

#### 3.4.2 ANCOM-BC LFC 可视化
...
if (nrow(ancombc_species) > 0) {
  ...
  p_ancombc <- ggplot(ancombc_plot_data,
    ...
    labs(
      ...
      y = NULL,
      fill = "Direction",
      caption = "Dashed lines: |LFC| = 1 threshold; * q < 0.05, † q < 0.1"
    ) +
  ...
  cat("✅ ANCOM-BC LFC 图已保存: 23_daa_ancombc_lfc_barplot.png\n")
}
```


# Part 2: Virus多样性分析 {#sec-virus}

本部分分析病毒组数据，方法与Part 1相同，用于比较细菌和病毒对CagA感染的响应差异。
...
# Part 3: 2×3因子设计扩展分析 - Standard {#sec-factorial-standard}

本部分将分析从核心双组比较扩展为完整的2×3因子设计，验证CagA效应在不同遗传背景下的一致性。

## 分析目的

- 验证G×E（基因型×环境）交互作用
- 确认CagA效应是否依赖于Apc突变背景
...
if (file.exists(here::here("data", params$name, "48_part4_effect_summary.csv")) &&
  ...
  pairwise_df_en$question <- dplyr::case_when(
    pairwise_df_en$question == "CagA效应@变异鼠" ~ "CagA effect @ApcMUT",
    pairwise_df_en$question == "CagA效应@野生型" ~ "CagA effect @ApcWT",
    pairwise_df_en$question == "Apc效应@CagA+感染" ~ "Apc effect @CagA+",
    pairwise_df_en$question == "Apc效应@CagA-感染" ~ "Apc effect @CagA-",
    pairwise_df_en$question == "感染效应@变异鼠(CagA+)" ~ "Infection @ApcMUT(CagA+)",
    pairwise_df_en$question == "感染效应@变异鼠(CagA-)" ~ "Infection @ApcMUT(CagA-)",
    TRUE ~ pairwise_df_en$question
  )

  # 1. PCoA 图 (使用已保存的 tse_full)
  ...
  cat("\n✅ Part 4 英文版总结图已保存: 47_part4_summary_figure_EN.png\n")
}
```
...
# ------------------------------------------------------------------------------
# Part 4: 分层成对比较 - Standard {#sec-pairwise-standard}
# 共9个比较，分为3类科学问题
# ------------------------------------------------------------------------------
...
# Part 5: 2×3因子设计扩展分析 - Virus {#sec-factorial-virus}
...
# Part 6: 分层成对比较 - Virus {#sec-pairwise-virus}
...
cat("
## Part 1: Alpha & Beta多样性分析 - 核心比较结果

| 分析 | 数据类型 | 指标 | P值 | 结论 |
|------|----------|------|-----|------|
| Alpha多样性 | Standard | Shannon | 0.73 | NS - 群落复杂度无变化 |
| Alpha多样性 | Standard | Observed | 0.29 | NS |
| **Beta多样性** | **Standard** | **Bray-Curtis PERMANOVA** | **0.012** | **显著 - 群落结构重塑** |
| Beta多样性 | Virus | Bray-Curtis PERMANOVA | 0.032 | 显著 - 病毒组也响应 |

## 关键结论

1. **Alpha多样性保持稳定**：CagA感染未导致菌群多样性崩溃，排除了'大灭绝'假说
2. **Beta多样性显著改变**：群落组成发生显著重塑，支持Functional Footprint假说
3. **Standard与Virus差异响应**：细菌响应更强（P=0.012 vs 0.032）
4. **2×3因子设计验证**：G×E交互作用确认CagA效应依赖Apc突变背景

## 与假说的关系

本分析为 **Functional Footprint假说** 提供了强有力支持：
- CagA来了又走了，但留下了群落结构的持久改变
- 这种改变可能是CagA特异性T细胞持续活化的基础
")
```
...
1. **Part 2 (02_functional_profiling.qmd)**: 功能谱分析 - 检验功能层面是否也有足迹
2. **Part 3 (03_singlem_diversity_analysis.qmd)**: SingleM/Lyrebird多样性 - 独立验证
3. **差异丰度分析**: 具体哪些物种驱动了Beta多样性差异
