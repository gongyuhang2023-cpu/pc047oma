...
```{r}
#| label: part6-phage-ko-correlation
...
```{r}
#| label: part6-phage-pathway-correlation
...
```{r}
#| label: part6-phage-pathway-heatmap
#| fig-width: 14
#| fig-height: 10
...
```{r}
#| label: part6-bacteria-correlations
...
```{r}
#| label: part6-tripartite-network
#| fig-width: 14
#| fig-height: 12
...
if (nrow(edges_df) > 0) {
  ...
  # 绘制网络 (设置 seed 和 repel 参数避免 render 卡住)
  set.seed(42)
  ...
  # 不用 print()，避免 knitr 捕获图形（可能导致 render 卡住）
  # 直接用 ggsave 保存，然后在下一个代码块用 include_graphics 导入
...
```{r}
#| label: part6-tripartite-network-display
#| echo: false

# 导入刚才 ggsave 保存的图片
if (exists("network_generated") && network_generated) {
  knitr::include_graphics(here::here("data", params$name, "66_tripartite_network.png"))
}
```
