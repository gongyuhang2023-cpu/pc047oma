...
# 检查噬菌体TSE是否存在
phage_file <- here::here("analyses", "data", "03_singlem_diversity_analysis",
                          "tse_lyrebird.rds")
...
if (has_phage) {
  common_samples <- intersect(colnames(tse_bacteria), colnames(tse_phage))
...
# 转换为相对丰度
bacteria_relab <- sweep(bacteria_filtered, 2, colSums(bacteria_filtered), "/")
```
...
for (grp in groups_to_compare) {
  ...
  grp_cor <- cor(t(grp_relab), method = "spearman")
...
if (ecount(g_all) > 0) {
  # 计算中心性指标
  degree_cent <- degree(g_all)
  betweenness_cent <- betweenness(g_all)
  closeness_cent <- closeness(g_all)
