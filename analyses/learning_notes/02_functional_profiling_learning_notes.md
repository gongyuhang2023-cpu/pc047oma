...
2. **Stratified数据分析**：观察功能的物种贡献者是否改变 → **Part 9 已完成**
3. **代谢组学验证**：直接测定代谢物水平
4. **株系水平分析**：使用StrainPhlAn检测株系变异

---

## 10. Part 9 扩展分析 - 分层功能分析（Stratified Analysis）

### 10.1 分析背景与目的

Part 8 发现：物种 Beta 多样性交互作用显著（p=0.024），但功能 Beta 多样性交互作用不显著（p=0.223）。

**关键问题**：虽然功能总量不变，但**执行这些功能的物种（Driver Species）是否发生了更替**？

这是 OMA Book 和 HUMAnN 协议推荐的深入分析：
- 使用 `*_stratified.tsv` 数据
- 追溯每个通路的物种贡献者
- 验证"功能冗余"假说的具体机制
...
```r
# 1. 读取分层数据（Pathway|Species 格式）
stratified_raw <- fread("humann4_pathabundance-step2-normalized_stratified.tsv")

# 2. 解析通路-物种对
# 格式: "PWY-7219: adenosine biosynthesis|s__Bacteroides_uniformis.t__SGB27814"

# 3. 计算物种对通路的贡献比例
contribution_pct = species_abundance / pathway_total * 100

# 4. 比较 ApcMUT 背景下 HpWT vs HpKO 的 Driver 变化
driver_comparison <- ... %>%
...
| Multiple | 多个通路 | *Microbacterium sp T32* 变化 | 变化 |

### 10.4 生物学意义

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  "静默更替"（Silent Shift）现象                                              │
│                                                                              │
│  功能总量：稳定 (功能Beta p=0.223)                                           │
│  执行者身份：发生重大更替                                                     │
│                                                                              │
│  ┌─────────────┐                  ┌─────────────┐                           │
│  │   HpKO组    │    →    →    →   │   HpWT组    │                           │
│  │  (CagA-)    │                  │  (CagA+)    │                           │
│  ├─────────────┤                  ├─────────────┤                           │
│  │ Lactobacillus │  ──减少──>     │ Mucispirillum │                         │
│  │ Ligilactobacillus │  ──减少──> │ (炎症相关)   │                          │
│  │  (益生菌)   │                  │             │                           │
│  └─────────────┘                  └─────────────┘                           │
│                                                                              │
│  结论：CagA感染导致功能的"执行者"从益生菌转向潜在致病菌                       │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 10.5 免疫学启示

**为什么物种更替可能解释 CD8+ T 细胞持续激活？**

| 机制 | 说明 |
|------|------|
| **差异 LPS 结构** | 不同物种的脂多糖结构不同，激活不同的免疫反应 |
| **肽聚糖变化** | *Lactobacillus* 减少意味着细胞壁成分改变 |
| **炎症菌增加** | *Mucispirillum schaedleri* 与肠道炎症相关 |
| **分子模拟** | 新定殖物种可能提供与 CagA 交叉反应的抗原 |

### 10.6 对假说的最终验证

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  "Functional Footprint" 假说 - 完整证据链                                    │
│                                                                              │
│  Step 1: 物种组成改变                                                        │
│    ✓ 01 Part 4: Beta交互作用 p=0.024                                        │
│    ✓ CagA效应@ApcMUT: p=0.007, R²=33.6%                                     │
│                                                                              │
│  Step 2: 功能冗余                                                            │
│    ✓ 02 Part 8: 功能Beta交互作用 p=0.223（不显著）                          │
│    ✓ 整体代谢潜力保持稳定                                                    │
│                                                                              │
│  Step 3: Driver 更替（Part 9 新发现）                                        │
│    ✓ 165个通路-物种组合发生>15%贡献变化                                      │
│    ✓ 益生菌（Lactobacillus）贡献减少                                         │
│    ✓ 潜在致病菌（Mucispirillum）贡献增加                                     │
│                                                                              │
│  结论：功能冗余掩盖了潜在的免疫原性改变                                       │
│        T细胞激活可能源于物种更替带来的免疫刺激变化                            │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 文件 | 内容 |
|------|------|
| `91_part9_driver_species_shift.png` | Driver更替可视化（堆叠柱状图） |
| `92_part9_driver_species_tests.csv` | 统计检验结果 |
| `93_part9_stratified_analysis_results.rds` | 完整分析数据对象 |

---

*2026-01-17 更新 | Part 9 分层功能分析*

---

## 11. Part 10 扩展分析 - 9个成对比较验证功能冗余（2026-01-19 新增）

### 11.1 分析目的

Part 10 与 01 分析的 Part 5 完全对应，对功能通路进行相同的 9 个系统性成对比较，以：
1. 验证物种层面的发现是否在功能层面重现
2. 量化功能冗余程度
3. 识别"物种-功能一致"vs"功能冗余"的比较

### 11.2 功能层面 9 个成对比较结果

| 比较 | R² | P值 | P_adj | 显著性 |
|------|-----|-----|-------|--------|
| Apc基线效应 (Ctrl组) | 3.9% | 0.617 | 0.638 | - |
| **Apc效应 (HpKO感染后)** | **49.6%** | **0.009** | **0.041** | ⭐⭐ |
| Apc效应 (HpWT感染后) | 18.1% | 0.074 | 0.167 | - |
| 感染效应 (ApcMUT, 无CagA) | 37.4% | 0.068 | 0.167 | - |
| 感染+CagA效应 (ApcMUT) | 12.2% | 0.451 | 0.638 | - |
| **CagA效应 (ApcMUT背景)** | **33.1%** | **0.007** | **0.041** | ⭐⭐ |
| 感染效应 (ApcWT, 无CagA) | 5.0% | 0.535 | 0.638 | - |
| 感染+CagA效应 (ApcWT) | 3.6% | 0.638 | 0.638 | - |
| CagA效应 (ApcWT背景) | 19.0% | 0.132 | 0.238 | - |

**关键发现**：
- FDR校正后显著的比较：**2/9** (22.2%)
- 核心比较 CagA效应@ApcMUT：**P_adj=0.041, R²=33.1%** ⭐

### 11.3 物种-功能冗余对比分析

| 比较 | 物种 R² | 物种 P_adj | 功能 R² | 功能 P_adj | **冗余模式** |
|------|---------|-----------|---------|-----------|-------------|
| Apc基线效应 | 21.8% | 0.181 | 3.9% | 0.638 | 均无差异 |
| Apc效应 (HpKO后) | **37.6%** | **0.039** | **49.6%** | **0.041** | **物种-功能一致** |
| Apc效应 (HpWT后) | **27.3%** | **0.039** | 18.1% | 0.167 | **功能冗余** |
| 感染效应 (ApcMUT) | 25.9% | 0.066 | 37.4% | 0.167 | 其他模式 |
| 感染+CagA (ApcMUT) | 13.4% | 0.507 | 12.2% | 0.638 | 均无差异 |
| **CagA效应 (ApcMUT)** | **33.6%** | **0.039** | **33.1%** | **0.041** | **物种-功能一致** ⭐ |
| 感染效应 (ApcWT) | **24.2%** | **0.043** | 5.0% | 0.638 | **功能冗余** |
| 感染+CagA (ApcWT) | 25.1% | 0.054 | 3.6% | 0.638 | 其他模式 |
| CagA效应 (ApcWT) | 13.7% | 0.387 | 19.0% | 0.238 | 均无差异 |
...
│                                                                              │
│  物种-功能一致：2个比较（22%）                                                │
│    - Apc效应 (HpKO后): 物种37.6%, 功能49.6%, 都显著                          │
│    - CagA效应 (ApcMUT): 物种33.6%, 功能33.1%, 都显著 ⭐核心                   │
│                                                                              │
│  功能冗余：2个比较（22%）                                                     │
│    - Apc效应 (HpWT后): 物种显著(0.039), 功能不显著(0.167)                    │
│    - 感染效应 (ApcWT): 物种显著(0.043), 功能不显著(0.638)                    │
│                                                                              │
│  均无差异：3个比较（33%）                                                     │
│  其他模式：2个比较（22%）                                                     │
└─────────────────────────────────────────────────────────────────────────────┘
...
### 11.5 核心发现：CagA效应的物种-功能一致性

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  重要发现：核心比较（CagA效应@ApcMUT）呈现物种-功能一致性                      │
│                                                                              │
│  物种层面：P=0.039, R²=33.6%  ✓ 显著                                         │
│  功能层面：P=0.041, R²=33.1%  ✓ 显著                                         │
│                                                                              │
│  这意味着：                                                                   │
│  1. CagA不仅改变物种组成，也改变功能组成                                      │
│  2. 功能冗余假说在核心比较中不成立                                            │
│  3. CagA效应是"真实的"生态和代谢影响                                         │
│                                                                              │
│  结合 Part 9 发现：                                                          │
│  - 虽然功能总量相似，但功能的"执行者"发生了更替                               │
│  - 益生菌减少，潜在致病菌增加                                                 │
│  - 这可能解释 CD8+ T 细胞的持续激活                                          │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 11.6 CagA效应的遗传背景依赖性（功能层面）

| 遗传背景 | 功能 R² | 功能 P_adj | 对比物种 P_adj |
|----------|---------|-----------|---------------|
| **ApcMUT** | **33.1%** | **0.041** | **0.039** |
| ApcWT | 19.0% | 0.238 | 0.387 |

**结论**：G×E交互作用在功能层面也存在！CagA功能效应同样依赖Apc突变背景。

### 11.7 02分析扩展结论汇总

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  02分析完整结论（2026-01-19更新）                                            │
│                                                                              │
│  核心发现：                                                                   │
│                                                                              │
│  1. 核心比较（CagA@ApcMUT）物种-功能一致                                      │
│     - 物种: P=0.039, R²=33.6%                                                │
│     - 功能: P=0.041, R²=33.1%                                                │
│     → CagA真正改变了肠道微生物的代谢功能                                      │
│                                                                              │
│  2. 部分比较呈现功能冗余                                                      │
│     - Apc效应(HpWT后): 物种变但功能不变                                       │
│     - 感染效应(ApcWT): 物种变但功能不变                                       │
│     → 说明功能冗余确实存在，但不是普遍现象                                    │
│                                                                              │
│  3. 功能层面G×E交互作用                                                       │
│     - ApcMUT: CagA功能效应显著(P=0.041)                                      │
│     - ApcWT: CagA功能效应不显著(P=0.238)                                     │
│     → 与物种层面一致                                                         │
│                                                                              │
│  4. 结合Part 9 Driver更替                                                    │
│     → 功能数量稳定，但执行者改变（益生菌→致病菌）                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 文件 | 内容 |
|------|------|
| `100_part10_all_pairwise_comparisons.csv` | 功能层面9个比较结果 |
| `101_part10_species_function_comparison.csv` | 物种-功能冗余对比 |
| `102_part10_caga_effect_comparison.png` | CagA效应对比可视化 |
| `103_part10_summary.rds` | 完整分析数据对象 |

---

*2026-01-19 更新 | Part 10 成对比较验证功能冗余*
