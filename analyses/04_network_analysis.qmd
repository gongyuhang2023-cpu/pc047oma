...
if (igraph::ecount(g_all) > 0) {
  ...
  # 模块检测
  communities <- igraph::cluster_louvain(g_all)
...
if (has_phage) {
  ...
  if (nrow(sig_pairs) > 0) {
    ...
    write.csv(cross_pairs_df,
              file = here::here("data", params$name, "11_bacteria_phage_associations.csv"),
              row.names = FALSE)
  } else {
...
if (has_phage) {
  ...
  write.csv(bact_phage_stats_df,
            file = here::here("data", params$name, "12_bacteria_phage_network_stats.csv"),
            row.names = FALSE)
}
```
...
if (has_phage) {
  ...
  ggsave(
    filename = here::here("data", params$name, "13_bacteria_phage_modularity_comparison.png"),
...
if (has_phage) {
  ...
  png(here::here("data", params$name, "14_bacteria_phage_network_2x2.png"),
  ...
  cat("2x2 网络对比图已保存: 14_bacteria_phage_network_2x2.png\n")
}
```
...
cat("2. 差异网络分析（NetShift）量化组间网络重构\n")
...
if (has_phage) {
  cat("  - 11_bacteria_phage_associations.csv\n")
  cat("  - 12_bacteria_phage_network_stats.csv\n")
  cat("  - 13_bacteria_phage_modularity_comparison.png\n")
  cat("  - 14_bacteria_phage_network_2x2.png\n")
}
```
