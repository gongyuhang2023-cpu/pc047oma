...
# 从01文档保存的TSE中读取元数据
tse_species <- readRDS(
  here::here("data", "01_alpha_beta_diversity_analysis", "tse_standard_species_ca_cleaned.rds")
)

# 提取元数据
metadata <- as.data.frame(colData(tse_species))
...
# 查看分组信息
cat("\n样本分组:\n")
print(table(metadata$cag_hpstatus))
```
...
# 加载元数据（与Part 1一致，从已保存的TSE中读取）
# 这确保了treatment_group等处理后的列存在
tse_species_for_ko <- readRDS(
  here::here("data", "01_alpha_beta_diversity_analysis", "tse_standard_species_ca_cleaned.rds")
)
sample_metadata_ko <- as.data.frame(colData(tse_species_for_ko))
sample_metadata_ko$sample_id <- rownames(sample_metadata_ko)
