...
**分析人员**: Gong Yuhang
**版本**: v4.1 (2026-01-17) - 新增Part 9分层功能分析
...
2. **功能分析**显示功能Beta多样性交互作用不显著（P=0.223），与物种水平形成鲜明对比，呈现典型的**功能冗余现象**。然而，**分层功能分析(Stratified Analysis)**揭示了165个通路-物种组合发生>15%的贡献变化，证实功能虽保持稳定，但**执行者（Driver Species）已从益生菌转向潜在致病菌**。
...
#### 3.1.4 扩展分层成对比较分析（Part 5/10新增）

为了更全面地验证G×E交互作用并探究"感染抹平Apc效应"等假设，我们对全部6组进行了系统性的9个成对比较分析：

**表4b. 扩展分层成对比较设计**

| 类别 | 比较内容 | 科学问题 |
|------|----------|----------|
| **Apc效应** | MUT vs WT @ Ctrl/HpKO/HpWT | 感染是否"抹平"Apc基因型差异？ |
| **感染效应@ApcMUT** | Ctrl→HpKO→HpWT | ApcMUT背景下的感染梯度效应 |
| **感染效应@ApcWT** | Ctrl→HpKO→HpWT | ApcWT背景下的感染梯度效应 |

此分析在三个层面同步进行：
- **01分析Part 5**: 物种Beta多样性 (Bray-Curtis)
- **02分析Part 10**: 功能Beta多样性 (MetaCyc通路)
- **03分析Part 5**: 系统发育Beta多样性 (SingleM/Lyrebird)

**预期结果**:
1. 若Apc效应在Ctrl组显著但在感染组消失 → 支持"感染抹平Apc效应"假设
2. 若CagA效应在ApcMUT显著但在ApcWT不显著 → 验证G×E交互作用
3. 物种显著但功能不显著的比较 → 支持功能冗余假说

> **注**: 此扩展分析的具体结果需运行更新后的qmd文件获得，结果将自动保存至各分析的`data/`目录下。
...
| Step 2b: Driver更替 | 执行者物种改变 | **165个通路-物种组合变化>15%**，益生菌↓致病菌↑ | ✓ **新发现** |
| Step 3: 代谢物驱动T细胞 | 特定代谢物关联 | 缺少数据 | ○ **待验证** |
| Step 4: T细胞促肿瘤 | 表型关联 | 缺少数据 | ○ **待验证** |

**结论**: Part 9的分层分析为假说提供了新的机制解释。虽然功能总量不变（功能冗余），但**执行者从益生菌转向潜在致病菌**。这种"静默更替"可能通过改变免疫刺激谱来解释T细胞的持续激活。假说从"代谢功能改变"调整为"**抗原来源改变**"。
