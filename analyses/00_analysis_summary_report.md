...
**分析人员**: Gong Yuhang
**版本**: v8.0 (2026-01-28) - 整合网络分析至04，新增Bootstrap稳定性、Zi-Pi分析、SparCC分析
...
### 3.5 网络分析：CagA破坏群落模块化结构（04分析 - 2026-01-28更新）

共现网络(Co-occurrence Network)分析揭示了CagA感染如何改变菌群物种间的相互作用模式。04分析整合了原03c内容，并新增Bootstrap稳定性分析、Zi-Pi关键物种分析和SparCC组成型相关分析。

#### 3.5.1 网络拓扑结构变化

**表14. 细菌共现网络统计（Top 50高丰度物种，|r|≥0.5）**

| 指标 | ApcMUT_HpKO (对照) | ApcMUT_HpWT (CagA+) | 变化 | 生物学意义 |
|------|-------------------|---------------------|------|------------|
| 节点数 | 50 | 50 | 0 | 相同物种集合 |
| 边数 | 416 | 360 | **-56 (-13%)** | 物种间关联减少 |
| 网络密度 | 0.340 | 0.294 | -0.046 | 连接稀疏化 |
| **模块度** | **0.393** | **0.237** | **-0.156 (-40%)** | **模块结构削弱** |
...
**表15a. Bootstrap稳定Hub物种（Bootstrap 1000次，稳定性>70%）**

| 物种 | 连接度 | 介数中心性 | 功能特征 |
|------|--------|------------|----------|
| *Blautia parvula* | 26 | 65.2 | SCFA生产者 |
| *Marvinbryantia formatexigens* | 24 | 39.0 | 甲酸利用 |
| *Roseburia hominis* | 24 | 46.0 | 丁酸生产者 |
| *Sellimonas catena* | 23 | 35.3 | 厌氧菌 |
| *Hungatella hathewayi* | 19 | 19.3 | 厌氧发酵 |

**特征**：经Bootstrap稳定性验证的Hub物种主要是**厌氧梭菌类**（Blautia、Roseburia、Hungatella），是肠道短链脂肪酸(SCFA)的主要生产者。

#### 3.5.3 Zi-Pi分析：节点生态角色分类

Zi-Pi分析根据模块内连接度(Zi)和模块间参与度(Pi)将物种分为四类角色：

| 角色 | 定义 | 数量 | 代表物种 |
|------|------|------|----------|
| Module hub | Zi>2.5, Pi<0.62 | 0 | - |
| Network hub | Zi>2.5, Pi≥0.62 | 0 | - |
| Connector | Zi≤2.5, Pi≥0.62 | 0 | - |
| **Peripheral** | Zi≤2.5, Pi<0.62 | **100%** | 全部物种 |

**关键发现**：所有节点都是Peripheral角色，没有检测到严格定义的Keystone物种（Zi>2.5）。这表明在当前样本量下，网络中没有形成高度中心化的模块结构，符合CagA导致模块度降低的发现。

#### 3.5.4 SparCC组成型相关性分析

SparCC (Sparse Correlations for Compositional Data)专门处理组成型数据的相关性计算：

**表15b. Spearman vs SparCC网络比较**

| 方法 | 阈值 | 节点数 | 边数 | 密度 |
|------|------|--------|------|------|
| Spearman | |r|≥0.75 | 100 | 530 | 0.107 |
| SparCC | |r|≥0.50 | 50 | 409 | 0.334 |

**解读**：SparCC使用更低的阈值即可获得相似规模的网络，表明其能更有效地识别真实的组成型数据相关性。
...
| 数据类型 | 组别 | 边数 | 模块度 | 变化 |
|----------|------|------|--------|------|
| **细菌** | Control (HpKO) | 511 | **0.433** | - |
| **细菌** | CagA+ (HpWT) | 512 | **0.160** | **-63%** |
| 噬菌体 | Control (HpKO) | 593 | 0.365 | - |
| 噬菌体 | CagA+ (HpWT) | 381 | 0.255 | -30% |

**核心发现**：CagA对细菌网络模块度的影响（-63%）远强于对噬菌体网络的影响（-30%），表明CagA主要通过破坏细菌群落的模块化结构影响肠道微环境。
...
│                                                                              │
│  新增验证分析（2026-01-28）：                                                  │
│  - Bootstrap稳定性分析确认Hub物种可靠性                                        │
│  - Zi-Pi分析显示无Keystone物种（模块结构瓦解的另一证据）                        │
│  - SparCC组成型分析验证相关性结果稳健性                                        │
│  - 细菌-噬菌体对比显示CagA主要影响细菌网络                                     │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
**表16. "Functional Footprint"假说验证状态（2026-01-28更新）**

| 假说步骤 | 预期结果 | 实际结果 | 状态 |
|----------|----------|----------|------|
| Step 1: 群落结构改变 | Beta多样性显著 | P=0.012 (物种), P=0.008 (UniFrac) | ✓ **完全支持** |
| Step 1b: 网络结构改变 | 互作模式改变 | 细菌模块度降低63%（04分析），Bootstrap稳定Hub物种确认 | ✓ **验证加强** |
| Step 1c: 细菌-噬菌体差异 | 双域协同变化 | 细菌模块度↓63% vs 噬菌体↓30%，CagA主要影响细菌 | ✓ **新发现** |
...
**结论**:
1. **群落结构全面重塑**：物种组成改变(Beta P=0.012)、细菌网络模块度降低63%、Driver物种更替
2. **细菌主导效应**：CagA对细菌网络影响(模块度↓63%)远强于噬菌体(↓30%)，Bootstrap/Zi-Pi/SparCC多方法验证
3. **功能冗余全面验证**：从Pathway→KO→GO→PFAM，各层面FDR校正后均无显著差异
4. **机制假说调整**：T细胞持续激活可能源于**菌群抗原谱改变**（物种更替+网络重组）而非代谢功能改变
...
2. **群落网络结构瓦解**: 网络分析显示CagA感染导致细菌**模块度降低63%**（0.433→0.160），群落从模块化组织变为混合状态；噬菌体网络受影响较小（降低30%）
...
| **04分析** | **`01_bacteria_network_stats.csv`** | **网络拓扑统计（含模块度）** |
| **04分析** | **`04_bacteria_hub_species.csv`** | **Hub物种中心性指标** |
| **04分析** | **`05_edge_stability_distribution.png`** | **Bootstrap边稳定性分布** |
| **04分析** | **`06_robust_hub_species.csv`** | **Bootstrap稳定Hub物种** |
| **04分析** | **`07_zi_pi_analysis.csv`** | **Zi-Pi分析结果** |
| **04分析** | **`08_zi_pi_plot.png`** | **Zi-Pi散点图** |
| **04分析** | **`12_sparcc_correlation_matrix.csv`** | **SparCC相关矩阵** |
| **04分析** | **`15_bacteria_phage_network_stats.csv`** | **细菌-噬菌体网络统计** |
| **04分析** | **`16_bacteria_phage_modularity_comparison.png`** | **模块度比较图** |
...
**文档维护**: Gong Yuhang | **项目代号**: PC047 vCagAepitope | **版本**: v8.0
